...
namespace LongMath {
    ...
    private:
        ...
        // Вспомогательные методы для преобразования в десятичное число
    ...
    // Преобразование в строку с десятичным представлением
    ...
    std::string LongNumber::toBinaryString() const {
        ...
        for (int i = bits.size() - 1; i >= 0; --i) {
            if (i == point) binaryString += '.';  // Точка перед дробной частью
            binaryString += bits[i] ? '1' : '0';
        }

        if (point == 0) binaryString += '0';  // Гарантия корректного вывода
    ...
    std::ostream &operator<<(std::ostream &os, const LongNumber &number) {
        return os << number.toDecimalString();  // Выводим десятичное представление
    }
    ...
    // Умножение по битам (аналог длинного умножения в столбик)
